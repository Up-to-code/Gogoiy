...
struct GameEngine: Sendable {
    ...
    private mutating func makePlayableTray(for board: Board) -> Tray {
        for _ in 0..<64 {
            let pieces = (0..<3).map { _ in makePiece() }
            if pieces.contains(where: { board.hasPlacement(for: $0) }) {
                return Tray(slots: pieces.map(Optional.some))
            }
        }

        let color = BlockColor.allCases.randomElement(using: &random) ?? .cyan
        let fallback = Piece(shape: PieceCatalog.single, color: color)
        return Tray(slots: [fallback, makePiece(), makePiece()].map(Optional.some))
    }

    private mutating func makePiece() -> Piece {
        let totalWeight = PieceCatalog.shapes.reduce(0) { $0 + $1.weight }
        var selection = Int.random(in: 0..<totalWeight, using: &random)
        let shape = PieceCatalog.shapes.first { shape in
